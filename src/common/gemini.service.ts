...
export class GeminiService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.logToFile(
      `--- Gemini Service Initialized at ${new Date().toISOString()} ---`,
    );
  ...
  private async callGemini(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logToFile(
          `[Attempt ${attempt}/${maxRetries}] Generating content...`,
        );
        ...
        if (response.usageMetadata) {
          this.logToFile(
            `[Tokens] Prompt: ${response.usageMetadata.promptTokenCount}, ` +
        ...
        if (error.message?.includes('API key')) {
          ...
          throw new Error(
            'Invalid or expired API key. Please update your GEMINI_API_KEY in .env',
          );
        }
  ...
  async generateQuestions(
    ...
    const additionalQuestionsNeeded = hasCustomQuestions
      ? Math.max(0, count - input.customQuestions.length)
      : count;
...
Example format: ["Question 1", "Question 2", "Question 3"]`;
    ...
    if (hasCustomQuestions) {
      this.logToFile(
        `[generateQuestions] Including ${input.customQuestions.length} custom questions`,
      );
    }
...
{"techScore": <number>, "commScore": <number>, "overfitScore": <number>, "aiFlagged": <boolean>, "feedback": "<brief critique>"}`;
    ...
    this.logToFile(
      `[rateTurn] Rating answer for: "${question.substring(0, 50)}..."`,
    );
    ...
    try {
      ...
      this.logToFile(
        `✅ Rated: Tech=${result.techScore}, Comm=${result.commScore}, Overfit=${result.overfitScore}, Flagged=${result.aiFlagged}`,
      );
...
Return ONLY valid JSON (no markdown):
{
  ...
  "metrics": [
    ...
    this.logToFile(
      `[evaluateInterview] Evaluating ${input.responses.length} responses for: ${input.jobTitle}`,
    );
    ...
    try {
      ...
      this.logToFile(
        `✅ Evaluation complete: Score=${result.overallScore}, Fit=${result.isFit}`,
      );
      ...
      return {
        ...
        isFit: false,
        reasoning:
          'Evaluation failed due to technical error. Manual review required.',
