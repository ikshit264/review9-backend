...
export class NotificationsService {
    ...
    async createForEmail(email: string, dto: Omit<CreateNotificationDto, 'email'>, sendEmail: boolean = true) {
        ...
        if (sendEmail && dto.type === NotificationType.EMAIL || dto.type === NotificationType.SYSTEM) {
    ...
    async bulkMail(dto: any) {
        ...
        const results = {
            ...
            succeeded: 0,
            failed: 0,
            details: [] as Array<{ email: string; status: 'success' | 'error'; error?: string }>,
        };
