...
import { SaveResponseDto, ProctoringLogDto } from './dto';
import { Status, CandidateStatus, NotificationType, Role, Plan } from '@prisma/client';
...
export class InterviewsService {
    ...
    async getInterviewByToken(token: string) {
        ...
        let candidate;
        if (token === 'test-me') {
            // Virtual token for automated testing
        ...
        if (candidate.status === CandidateStatus.INVITED && isExpired) {
            // Check if there's an ongoing session
            const ongoingSession = await this.prisma.interviewSession.findFirst({
                where: { jobId: candidate.jobId, candidate: { email: candidate.email }, status: Status.ONGOING },
            });
    ...
    async startInterview(dto: any, userId: string) {
        ...
        if (!user?.isProfileComplete) {
            throw new ForbiddenException('Please complete your profile before starting the interview');
        }
        ...
        if (now < startTime) {
            throw new ForbiddenException(`Interview has not started yet. It will begin at ${startTime.toLocaleString()}`);
        }

        if (now >= endTime) {
            throw new ForbiddenException('Interview window has expired. Please contact the company for a re-interview.');
        }
        ...
        if (candidate.status !== CandidateStatus.INVITED) {
            throw new ForbiddenException('You must accept the interview invitation before starting');
        }
        ...
        if (existingSession) {
            if (existingSession.status === Status.PAUSED) {
                throw new ForbiddenException('Interview is paused due to malpractice. Please contact the company to resume.');
            }
            if (existingSession.hasStarted && existingSession.status === Status.COMPLETED) {
                throw new ForbiddenException('Interview already completed');
            }
        ...
        if (resumeText) {
            ...
            await this.prisma.candidate.update({
                where: { id: candidate.id },
                data: { status: CandidateStatus.INVITED },
            });
        }
    ...
    async resumeInterview(sessionId: string, userId: string) {
        ...
        if (session.job.companyId !== userId) {
            throw new ForbiddenException('Only the company can resume this interview');
        }
    ...
    async respondToInterview(sessionId: string, answer: string, res: Response) {
        ...
        if (!session) throw new NotFoundException('Session not found');
        if (session.status !== Status.ONGOING) throw new ForbiddenException('Interview is not active');
        ...
        try {
            ...
            // In PRO/ULTRA, we also want to rate the answer
            let techScore = null;
            let commScore = null;
    ...
    async logProctoringEvent(sessionId: string, dto: ProctoringLogDto) {
        ...
        if (dto.severity === 'high') {
            ...
            if (newWarningCount >= 3) {
                // Direct Flagging/Failure on 3rd warning
                await this.prisma.interviewSession.update({
                    where: { id: sessionId },
                    data: {
                        isFlagged: true,
    ...
    async acknowledgeWarning(sessionId: string, userId: string) {
        ...
        if (!session) throw new NotFoundException('Session not found');
        if (session.candidateId !== userId) throw new ForbiddenException('Unauthorized');
    ...
    async completeInterview(sessionId: string) {
        ...
        // Update candidate status to REVIEW (interview completed, awaiting decision)
        if (candidate) {
            await this.prisma.candidate.update({
                where: { id: candidate.id },
    ...
    async getCandidateInvitations(email: string) {
        ...
        for (const invitation of invitations) {
            ...
            result.push({
                ...
                currentStatus: session ? (session.status === Status.COMPLETED ? 'COMPLETED' : session.hasStarted ? 'ONGOING' : session.status) : invitation.status
            });
        }
    ...
    async getOrCreateTestEnvironment(requestedPlan?: Plan) {
        ...
        if (!job) {
            job = await this.prisma.job.create({
                data: {
                    ...
                    interviewEndTime: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // Ends in 30 days
                }
            });
        ...
        if (!candidate) {
            ...
            // Update token to make it fresh or just use existing
            candidate = await this.prisma.candidate.update({
                where: { id: candidate.id },
                data: {
                    status: CandidateStatus.INVITED,
